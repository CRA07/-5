...
def webhook():
    ...
    if text.lower().startswith("#склад"):
        content_without_tag = text[6:].strip()  # убираем "#склад"
        ...
        # Запишем, даже если маркетплейса нет
    ...
    elif text.lower().startswith("#производство"):
        content_without_tag = text[13:].strip()  # убираем "#производство"

        found_product = find_match(content_without_tag, PRODUCTS)
        # Берём дефект из обоих списков
        found_complaint = find_match(content_without_tag, PRODUCTION_DEFECTS + WAREHOUSE_DEFECTS)
