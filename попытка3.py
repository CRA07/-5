...
PRODUCTION_DEFECTS = ["нет даты производства ", "волос в банке", "пустая банка", "банка без защитной фольги",
                      ...
                      "просрочка", "вскрытая банка", "пришел без этикетки", "вскрыт пакет с силикагелем",
                      "жалоба на плесень ", ]
    ...
    try:
        ...
        if text.startswith("#склад"):
            ...
            if not product:
                ...
                if success:
                    return jsonify({"success": "Message saved (product not )"}), 200
                else:
...
if __name__ == "__main__":
    ...
    if spreadsheet:
        ...
        logger.error("Проверьте наличие файла service-account.json")
        logger.error("Проверьте ID таблицы")
        logger.error("Дайте доступ к таблице для service account")
